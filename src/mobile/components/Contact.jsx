...
const Contact = () => {
  ...
  const contactDetails = [
    ...
    { id: 2, icon: 'fa-envelope', label: 'Secure Channel', value: 'hafizhassan142003@gmail.com', sub: '24/7 Response time', link: 'mailto:hafizhassan142003@gmail.com' },
    { id: 3, icon: 'fa-phone', label: 'Comms Link', value: '+92 322 4191962', sub: 'Mon - Sat (9am - 6pm)', link: 'tel:+923224191962' },
      ...
      <div className="container">
        ...
        <div className="contact-grid">
          <div className="contact-info-col">
            <div className="contact-badge-grid">
              {contactDetails.map((detail, i) => (
                <div key={detail.id} className="contact-badge-card" style={{ borderColor: `${contactColors[i]}25` }}>
                  <div className="badge-card-glow" style={{ background: `radial-gradient(circle at 50% 0%, ${contactColors[i]}12, transparent 70%)` }} />
                  <div className="badge-card-icon" style={{ color: contactColors[i], background: `${contactColors[i]}10` }}>
                    <i className={`fa-solid ${detail.icon}`}></i>
                  </div>
                  <div className="badge-card-info">
                    <span className="badge-card-label">{detail.label}</span>
                    {detail.link ? (
                      <a href={detail.link} target={detail.link.startsWith('http') ? '_blank' : undefined} rel={detail.link.startsWith('http') ? 'noreferrer' : undefined} className="badge-card-value link">{detail.value}</a>
                    ) : (
                      <span className="badge-card-value">{detail.value}</span>
                    )}
                  </div>
                  <span className="badge-card-sub">{detail.sub}</span>
                </div>
              ))}
            </div>
